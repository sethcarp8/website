...
document.addEventListener('scroll', () => {
  ...
  document.querySelector('.scroll-progress').style.setProperty('--scroll', `${scrollPercent}%`);
});
  ...
  if (document.querySelector('.blog-swiper')) {
    const initSwiper = () => {
      return new Swiper('.blog-swiper', {
        ...
        slidesPerView: 3,
        spaceBetween: 30,
        loop: false, // Changed to false for better search handling
        mousewheel: true,
        keyboard: {
          enabled: true,
        },
  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      articles.forEach(article => {
        ...
        article.closest('.swiper-slide').style.display = shouldShow ? 'block' : 'none';
      });
    });
  });
}
